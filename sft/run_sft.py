...
class ModelArguments:
    ...
    base_model_name_or_path: Optional[str] = field(
        ...
        metadata={
            "help": (
                "Path to the init sft checkpoint when evaluating a rlhf checkpoint"
            )
        }
    )
...
def main():
    ...
    config = AutoConfig.from_pretrained(
        model_args.config_name if model_args.config_name else (model_args.base_model_name_or_path if "rlhf" in model_args.model_name_or_path else model_args.model_name_or_path),
        cache_dir=model_args.cache_dir,
        revision=model_args.model_revision
    )
    tokenizer = AutoTokenizer.from_pretrained(
        model_args.tokenizer_name if model_args.tokenizer_name else (model_args.base_model_name_or_path if "rlhf" in model_args.model_name_or_path else model_args.model_name_or_path),
        cache_dir=model_args.cache_dir,
        use_fast=model_args.use_fast_tokenizer,
        revision=model_args.model_revision
    )

    if "rlhf" in model_args.model_name_or_path and training_args.do_predict:
        import torch
        ...
        if "best_policy" in model_args.model_name_or_path.split("/")[-1]:
            model.load_state_dict(torch.load(model_args.model_name_or_path))
        else:
            assert "ckp" in model_args.model_name_or_path.split("/")[-1], model_args.model_name_or_path.split("/")[-1]
            model.load_state_dict(torch.load(model_args.model_name_or_path)["model"])
    else:
        model = AutoModelForSeq2SeqLM.from_pretrained(
    ...
    def preprocess_function(examples):
        ...
        for i in range(len(examples[input_column])):
            # logger.info(i)
            # logger.info(examples[input_column][i])
            # logger.info(output_column)
            # logger.info(examples[output_column][i])
            # logger.info("\n\n")
        ...
        logger.info("==============={}===============".format(inputs[:10]))
        logger.info("---------------{}---------------".format(targets[:10]))
    ...
    if training_args.do_predict:
        ...
        if trainer.is_world_process_zero():
            if training_args.predict_with_generate:
                ...
                pred_file = os.path.join(training_args.output_dir, "{}_result.json".format(data_args.dataset))
