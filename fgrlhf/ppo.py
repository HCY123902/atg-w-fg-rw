...
class PPOTrainer:
    ...
    def train(self, step):
        ...
        if (step % self.args['train']['eval_interval']) < 1 and self.accelerator.is_main_process:
    ...
    def valid(self, step):
        ...
        with torch.no_grad():
            for i, batch in enumerate(tqdm(self.eval_dataloader) if self.accelerator.is_main_process else self.eval_dataloader):
                ...
                results = self.accelerator.gather_for_metrics({
                    'prompts_input_ids': results['prompts_input_ids'],
                    'generated_input_ids': results['generated_input_ids'],
                    'generated_attention_mask': results['generated_attention_mask'],
                    'generated_value': results['generated_value']
                })
                ...
                for eval_k, eval_v in eval_results.items():
                    ...
                    if len(eval_v) != 2 or any([not isinstance(x, (float, int)) for x in eval_v]):
                        raise Exception("{}: {}".format(eval_k, eval_v))
                ...
                if prev_corr_rec_best_step is None or mean_corr_rec > self.eval_corr_rec[prev_corr_rec_best_step]:
                    if prev_corr_rec_best_step is not None:
                        try:
                            ...
                            self.log_info(f'Cannot remove previous best ckpt!')
                    shutil.copy(f"{self.args['logging']['save_dir']}/last.pth", f"{self.args['logging']['save_dir']}/ckp_{step}_corr_rec.pth")
                    ...
                    if self.model_type == "llama":
                        ...
                        policy_model_state_dict = self.accelerator.unwrap_model(self.policy_model.model).state_dict()
                        self.accelerator.save(policy_model_state_dict, f"{self.args['logging']['save_dir']}/best_policy_corr_rec.pth")
    ...
    def save_llama(self, step):
        ...
        if self.accelerator.is_main_process:
            self.accelerator.unwrap_model(self.policy_model.model).save_pretrained(f"{self.args['logging']['save_dir']}/last_policy_peft")
            self.accelerator.unwrap_model(self.value_model.model).save_pretrained(f"{self.args['logging']['save_dir']}/last_value_peft")
